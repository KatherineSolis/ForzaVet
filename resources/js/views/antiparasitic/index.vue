    ...
    <el-row>
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span>Listado de antiparasitarios</span>
        </div>
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
          <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left">
            <el-form-item label="Nombre:" prop="name_antiparasitic">
              <el-input v-model="temp.name_antiparasitic" />
            </el-form-item>
...
<script>
import { fetchList, createAntiparasitaric, updateAntiparasitaric } from '@/api/antiparasitaric';
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          ...
          this.temp.status = 1;
          createAntiparasitaric(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          console.log(this.temp);
          updateAntiparasitaric(this.temp).then(() => {
    ...
    async handleModifyStatus(row, status) {
      ...
      row.status = status;
      console.log(row);
      await updateAntiparasitaric(row);
